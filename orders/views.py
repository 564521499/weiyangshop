...
class OrderProcessView(View):
    ...
    def get_address(self,request):
        ...
        """
        user = request.myuser
        addresses = Address.objects.filter(uid=user)
        addresses_list = []
    ...
    def get_cart_dict(self,request):
        ...
        cart_dict = {}
        # cart_dict = {sku_id:state:{count:xxx,select:xx}}
    ...
    def get_order_string(self,order_id,total_amount,sku_goods):
        ...
        alipay = AliPay(
            ...
            # 使用的文件读取方式,载入支付秘钥
            app_private_key_path= settings.ALI_PAY_KEY_PATH + "s7_private_key.pem",
            # 支付宝的公钥，验证支付宝回传消息使用，不是你自己的公钥,
            # 使用文件读取的方式,载入支付报公钥
            alipay_public_key_path= settings.ALI_PAY_KEY_PATH + "alipay_public_key.pem",
        ...
        order_string = alipay.api_alipay_trade_page_pay(
            ...
            notify_url="http://127.0.0.1:8000/v1/orders/success/"  # 可选, 不填则使用默认notify url
        )
        return order_string
    @logging_check
        ...
        if status == 0:
            ...
            with transaction.atomic():  # 禁止自动提交
                ...
                for sku in skus:
                    ...
                    # 使用乐观锁进行修改
                    # 使用乐观锁修改商品数量  MVCC  多版本号
                    version_old = sku.version   # 1
                    #  数据库里的SKU的版本号>1
